...
function CalcMode(this: CalcMode, output?: HTMLElement | DocumentFragment, win?: Window & typeof globalThis) {
    ...
    const sortButton = dom_getel(`#calc-sorter`, this.win.HTMLElement)
    if (sortButton) {
        sortButton.onclick = () => {
...
CalcMode.prototype.mkcontainers = function(this: CalcMode, into: HTMLElement | DocumentFragment) {
    ...
    return {
        container: c,
        output: dom_getelorthrow("#calc-items", null, c)
    }
}
...
CalcMode.prototype.chwin = function(this: CalcMode, win: Window & typeof globalThis) {
    ...
    this.output = dom_getelorthrow("#calc-items", null, container) as HTMLElement
